...
export const metadata: Metadata = {
  title: "Pricelist Sewa Motor Mahasiswa - Mas Karyo",
  description:
    "Daftar harga sewa motor untuk mahasiswa di Solo. Harga terjangkau khusus pelajar dan mahasiswa.",
};

export default async function PricelistMahasiswaPage() {
  const [motorcycles, settingsRecords] = await Promise.all([
    prisma.motorcycle.findMany({
      where: { isActive: true },
      include: {
        prices: {
          where: { isActive: true, category: { equals: "mahasiswa", mode: "insensitive" } },
        },
      },
      orderBy: { order: "asc" },
    }),
  ...
  return (
    <>
      ...
      <main className="flex-1 bg-white">
        <PricelistHeroClient
          badge="Pricelist Mahasiswa"
          title="Harga Spesial Khusus Mahasiswa"
          description="Nikmati harga spesial untuk pelajar dan mahasiswa. Syarat: tunjukkan KTM/Kartu Pelajar yang masih berlaku."
        />
        ...
        <AnimatedSection direction="up" className="mx-auto max-w-7xl px-4 pb-20 lg:pb-28">
          ...
          {bikes.length === 0 && (
            ...
            >
              Belum ada motor tersedia untuk kategori mahasiswa.
            </motion.p>
          )}
        </AnimatedSection>
      </main>
