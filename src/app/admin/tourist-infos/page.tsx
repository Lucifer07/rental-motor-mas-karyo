...
const emptyForm = { title: "", description: "", image: "", isActive: true };

export default function TouristInfosPage() {
  const [items, setItems] = useState<TouristInfo[]>([]);
  ...
  const fetchItems = useCallback(async () => {
    try {
      const res = await fetch("/api/tourist-infos");
      if (res.ok) setItems(await res.json());
    } catch {
      showToast("error", "Failed to fetch tourist infos");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const openEdit = (item: TouristInfo) => {
    setFormData({ title: item.title, description: item.description, image: item.image || "", isActive: item.isActive });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = isEditing ? `/api/tourist-infos/${selectedId}` : "/api/tourist-infos";
      ...
      if (res.ok) {
        showToast("success", `Tourist info ${isEditing ? "updated" : "created"}`);
        setShowModal(false);
        fetchItems();
      } else {
  ...
  const handleDelete = async (id: string) => {
    try {
      const res = await fetch(`/api/tourist-infos/${id}`, { method: "DELETE" });
      if (res.ok) { showToast("success", "Tourist info deleted"); fetchItems(); }
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Tourist Info</h1>
          <p className="text-gray-500 text-sm mt-1">Manage tourist information</p>
        </div>
        <button onClick={openAdd} className="bg-orange-500 text-white px-4 py-2.5 rounded-lg font-medium hover:bg-orange-600 transition flex items-center gap-2 text-sm">
          <Plus className="w-4 h-4" /> Add Tourist Info
        </button>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="p-4 border-b border-gray-100">
          <div className="relative">
            ...
            <input type="text" placeholder="Search tourist infos..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full sm:w-80 pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400"><MapPin className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>No tourist infos found</p></div>
        ) : (
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Delete Tourist Info</h3>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center pt-10 px-4 overflow-y-auto">
          <div className="bg-white rounded-xl max-w-lg w-full shadow-xl mb-10">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">{isEditing ? "Edit Tourist Info" : "Add Tourist Info"}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div>
                ...
                <textarea value={formData.description} onChange={(e) => setFormData((p) => ({ ...p, description: e.target.value }))} required rows={4} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none resize-none" />
              </div>
              <ImageUpload value={formData.image || ""} onChange={(url) => setFormData({...formData, image: url})} label="Gambar" />
