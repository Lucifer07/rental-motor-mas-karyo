...
export default function PromosPage() {
  ...
  const fetchItems = useCallback(async () => {
    try {
      ...
      showToast("error", "Failed to fetch promos");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = isEditing ? `/api/promos/${selectedId}` : "/api/promos";
      ...
      if (res.ok) {
        showToast("success", `Promo ${isEditing ? "updated" : "created"}`);
        setShowModal(false);
        fetchItems();
      } else {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (res.ok) { showToast("success", "Promo deleted"); fetchItems(); }
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="p-4 border-b border-gray-100">
          <div className="relative">
            ...
            <input type="text" placeholder="Search promos..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full sm:w-80 pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400"><Tag className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>No promos found</p></div>
        ) : (
      ...
      {deleteConfirm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
          <div className="bg-white rounded-xl p-6 max-w-sm w-full shadow-xl">
            <h3 className="text-lg font-semibold text-gray-900 mb-2">Delete Promo</h3>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center pt-10 px-4 overflow-y-auto">
          <div className="bg-white rounded-xl max-w-lg w-full shadow-xl mb-10">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">{isEditing ? "Edit Promo" : "Add Promo"}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <ImageUpload value={formData.image || ""} onChange={(url) => setFormData({...formData, image: url})} label="Gambar Promo" />
