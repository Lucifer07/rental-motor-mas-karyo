...
const emptyForm = { name: "", logo: "", order: 0, isActive: true };
...
export default function ClientsPage() {
  ...
  const openEdit = (item: Client) => {
    setFormData({ name: item.name, logo: item.logo, order: item.order, isActive: item.isActive });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      const url = isEditing ? `/api/clients/${selectedId}` : "/api/clients";
      ...
      if (res.ok) {
        showToast("success", `Client ${isEditing ? "updated" : "created"}`);
        setShowModal(false);
        fetchItems();
      } else {
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      if (res.ok) { showToast("success", "Client deleted"); fetchItems(); }
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage client logos</p>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100">
        <div className="p-4 border-b border-gray-100">
          <div className="relative">
            ...
            <input type="text" placeholder="Search clients..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full sm:w-80 pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-orange-500 focus:border-orange-500 outline-none" />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-gray-400"><Building2 className="w-12 h-12 mx-auto mb-3 opacity-50" /><p>No clients found</p></div>
        ) : (
                ...
                {filtered.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4">
                      {item.logo ? (
                        <img src={item.logo} alt={item.name} className="h-10 w-auto max-w-[100px] object-contain" />
                      ) : (
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center pt-10 px-4 overflow-y-auto">
          <div className="bg-white rounded-xl max-w-lg w-full shadow-xl mb-10">
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
              <h3 className="text-lg font-semibold text-gray-900">{isEditing ? "Edit Client" : "Add Client"}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <ImageUpload value={formData.logo} onChange={(url) => setFormData({...formData, logo: url})} label="Logo Klien" />
              <div>
